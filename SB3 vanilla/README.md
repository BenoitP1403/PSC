...
### `train.py`
Charge un scénario PrimAITE (par défaut `../Scenarios/data_manipulation_3_pc.yaml`), instancie un environnement `PrimaiteGymEnv`, puis entraîne l’algorithme sélectionné en ligne de commande en chargeant ses hyperparamètres optimisés depuis un fichier JSON (`../Parameter optimization/best_params/<ALGO>.json`). Les logs TensorBoard sont écrits dans `SB3 vanilla/tensorboard_logs/<ALGO>/` et le modèle final est sauvegardé sous `SB3 vanilla/<ALGO> model.zip`.
